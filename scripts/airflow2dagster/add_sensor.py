...
class AddSensorSignature(dspy.Signature):
    """Translate sensor(s) in the Airflow code into Dagster sensors"""

    context = dspy.InputField(desc="Potentially relevant Dagster documentation")
    airflow_code = dspy.InputField(desc="Airflow code containing sensor(s)")
    input_dagster_code = dspy.InputField(
        desc=(
            "Equivalent Dagster code without sensors. "
            f"Assume that it is located in a sibling Python module `{PLACEHOLDER_MODULE_FILENAME}`"
        )
    )
    sensor_code = dspy.OutputField(
        desc=(
            "Dagster sensor code that is equivalent to Airflow's, in a separate file. "
...
class AddSensorModule(dspy.Module):
    ...
    def forward(self, airflow_code: str, input_dagster_code: str) -> dspy.Prediction:
        ...
        pred.sensor_code = extract_code_block_from_markdown(pred.sensor_code)

        concatenated_code = combine_code_snippets(
            [input_dagster_code, pred.sensor_code]
        )
        is_runnable_bool, error_msg = is_runnable(concatenated_code, verbose=True)
        # HACK: Bypass obtuse error message from Dagster sensor
        if (
            """Error: Param "target" is not one of ['GraphDefinition', 'JobDefinition', 'UnresolvedAssetJobDefinition']"""
            in error_msg
        ):
            error_msg = (
                "The `@sensor(job=...)` argument is only applicable for Dagster jobs. "
                "Either supply an asset_job, or use `@sensor(asset_selection=[...])` to link the sensor to the asset(s) directly."
            )

        dspy.Suggest(
            is_runnable_bool, error_msg
